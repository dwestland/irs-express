...
from irs656.tasks import fill_form_656
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if sync:
            fill_form_656(client_id=options.get('client_id'), outfile=options.get('outfile'))
        else:
            fill_form_656.apply_async(kwargs={'client_id': options.get('client_id'),
